...
import { Star, Heart, ArrowRight, Truck, RefreshCw, Minus, Plus, X } from "lucide-react"
...
import { ProductById } from "@/sanity/types/product"
import { useState } from "react"
import { motion } from "framer-motion"


export default function ProductViewPage({ product }: { product: ProductById }) {
        ...
        <div className="min-h-screen bg-white font-sans overflow-hidden">
            {
                isModalOpen && (
                    <motion.div
                        ...
                        className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" onClick={() => setIsModalOpen(false)}>
                        <div className="absolute top-4 right-4 z-10 bg-white rounded-full p-2" onClick={() => setIsModalOpen(false)}>
                            <X className="w-8 h-8 text-red-500" />
                        </div>
                        <motion.div className="relative overflow-auto h-[500px] w-[500px]" onClick={(e) => e.stopPropagation()}>
            ...
            <main className="container mx-auto px-4 py-12">
                <div className="grid lg:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-4">
                        <Carousel className="w-full" id="main-carousel">
                            <CarouselContent>
                                {product.images.map((image, i) => (
                                    <CarouselItem key={i} id={`carousel-item-${i}`}>
                            ...
                            {product.images.map((image, i) => (
                                <div
                                    ...
                                    onClick={() => {
                                        ...
                                        const targetItem = document.getElementById(`carousel-item-${i}`);
                                        if (mainCarousel && targetItem) {
                                            const scrollOptions = {
                    ...
                    <div className="space-y-8">
                        <div>
                            <h1 className="text-5xl font-bold mb-2 tracking-tight">{product.productName}</h1>
